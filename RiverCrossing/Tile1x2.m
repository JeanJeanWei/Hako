            ...
            if (character.tag != self.tag)
            {
                ...
                if (CGRectIntersectsRect(myBoundingBox, characterBox))
                {
                    ...
                    if (!isnan(self.position.x) && !isnan(self.position.y)) {
                        CGPoint pos = [TileHelper.instance getClosest_1x2:self.position];
                        self.position = pos;
...
- (id)initWithPosition:(int)pos andSprite:(id)obj andEffect:(GameItem*)effect
{
    ...
    {
        is2x2 = NO;
        CGPoint p = [TileHelper.instance getPosition:pos forTile:k1x2];
        self.position = p;
        oldPosition = p;
        ...
        [self initAnimations];
        [self setDisplayFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"1x2_idle_1.png"]]; 
    }
    return self;
}
