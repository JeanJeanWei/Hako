...
{    
    ...
    //        return; // Currently playing the walking damage animation
...
//                NSLog(@"characterBox = %@", NSStringFromCGRect(characterBox));
                ...
                if (CGRectIntersectsRect(myBoundingBox, characterBox)) {
                    ...
                    {
                        CGPoint pos = [TileHelper.instance getClosest_1x1:self.position];
                        self.position = pos;
...
- (id)initWithPosition:(int)pos andSprite:(id)obj andEffect:(GameItem*)effect
{
    ...
    {
        is2x2 = NO;
        CGPoint p = [TileHelper.instance getPosition:pos forTile:k1x1];
        self.position = p;
        oldPosition = p;
        accessary = obj;
        ...
        [self initAnimations];
        [self setDisplayFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"1x1_idle_1.png"]];
    }
    return self;
}
